...
function redactDiagnosticText(value: string): string {
  ...
  return redacted.length <= 8_000 ? redacted : `${redacted.slice(0, 8_000)}…`;
}

function scalarDiagnostic(value: unknown): string | number | boolean | undefined {
  if (typeof value === "string") return redactDiagnosticText(value);
  if (typeof value === "number" || typeof value === "boolean") return value;
  return undefined;
}

function errorDiagnostic(error: unknown, depth = 0): Record<string, unknown> {
  if (!(error instanceof Error)) {
    return {
      type: typeof error,
      value: redactDiagnosticText(String(error)),
    };
  }
  ...
  const diagnostic: Record<string, unknown> = {
    ...
    message: redactDiagnosticText(error.message || "<empty>"),
  };
  const code = scalarDiagnostic(extended.code);
  const errno = scalarDiagnostic(extended.errno);
  const syscall = scalarDiagnostic(extended.syscall);
  const errorPath = scalarDiagnostic(extended.path);
  if (code !== undefined) diagnostic.code = code;
  if (errno !== undefined) diagnostic.errno = errno;
  if (syscall !== undefined) diagnostic.syscall = syscall;
  if (errorPath !== undefined) diagnostic.path = errorPath;
  if (error.stack) diagnostic.stack = redactDiagnosticText(error.stack);
  if (depth < 2 && extended.cause !== undefined) {
    diagnostic.cause = errorDiagnostic(extended.cause, depth + 1);
  }
  return diagnostic;
}

function logRuntimeFailure(stage: "create" | "start" | "send", error: unknown): void {
  process.stderr.write(
    `[live proof sdk ${stage} diagnostic: ${JSON.stringify(errorDiagnostic(error))}]\n`,
  );
}

class DiagnosticRuntimeFactory implements ClineRuntimeFactory {
  private readonly inner = new SdkClineRuntimeFactory();

  async create(request: Parameters<ClineRuntimeFactory["create"]>[0]) {
    let runtime;
    try {
      runtime = await this.inner.create(request);
    } catch (error) {
      logRuntimeFailure("create", error);
      throw error;
    }

    const originalStart = runtime.start.bind(runtime);
    runtime.start = async (input: unknown) => {
      try {
        return await originalStart(input);
      } catch (error) {
        logRuntimeFailure("start", error);
        throw error;
      }
    };

    const originalSend = runtime.send.bind(runtime);
    runtime.send = async (input: unknown) => {
      try {
        return await originalSend(input);
      } catch (error) {
        logRuntimeFailure("send", error);
        throw error;
      }
    };
...
async function parentMain(): Promise<void> {
  ...
  process.stdout.write(
    `[isolated live proof: workspace=${workspace.canonicalRoot}; clineRoot=${isolation.clineDir}; hub=${isolation.hubAddress}; registry=isolated; diagnostics=on]\n`,
  );
...
void main().catch((error) => {
  ...
  process.stderr.write(
    `[live owner-loss proof diagnostic: ${JSON.stringify(errorDiagnostic(error))}]\n`,
  );
  process.exitCode = 1;
});